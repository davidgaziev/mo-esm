...
namespace info.lundin.math
{
    ...
    public class TreeParser
    {
        ...
        ParseE(string exp)
        {
            ...
            string regEx = $"(\\d+([\\{decimalSeparator}]{{1}}\\d+)?)e([-+]?\\d+)";
            return Regex.Replace(exp, regEx, "($1*10^$3)");
        }
            ...
            while (i < len)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        farg = arg(fop, exp, i + fop.Length);
                        if (farg.Equals("")) throw new Exception("Wrong number of arguments to operator " + fop);
                        if (tree != null) throw new Exception("Missing operator");
                        tree = new Node(operators[fop], ParseInfix(farg));
                        i += farg.Length + fop.Length;
                    }
                }
        ...
        getOp(String exp, int index)
        {
            ...
            for (i = 0; i < maxoplength; i++)
            {
                if (index >= 0 && (index + maxoplength - i) <= len)
                {
                    ...
                    if (isOperator(tmp))
                    {
                        bool isFunc = !isTwoArgOp(tmp) && tmp != "!";
                        if (!isFunc || !bRequireParentheses || bImplicitMultiplication || (!bImplicitMultiplication && bRequireParentheses && isFunc && index + maxoplength - i < exp.Length && exp[index + maxoplength - i] == '('))
                        {
                            return (tmp);
                        }
                    }
